...
void ridmc_basin_slow_free(SEXP p);

SEXP ridmc_basin_slow_alloc(SEXP model, SEXP parms,
	SEXP xmin, SEXP xmax, SEXP xres,
	SEXP ymin, SEXP ymax, SEXP yres, 
	SEXP attractorLimit, SEXP attractorIterations, SEXP ntries) {
	SEXP ans;
	int ians;
	idmc_basin_slow *basin;
	
	ians = idmc_basin_slow_alloc(R_ExternalPtrAddr(model), REAL(parms), 
...
void ridmc_basin_slow_free(SEXP p) {
	...
	idmc_basin_slow_free( R_ExternalPtrAddr(p) );
}

SEXP ridmc_basin_slow_step(SEXP p) {
	int ians = idmc_basin_slow_step(R_ExternalPtrAddr(p));
	if(ians!=IDMC_OK)
		RIDMC_GENERIC_ERROR(ians);
	return R_NilValue;
}

SEXP ridmc_basin_slow_finished(SEXP p) {
	SEXP ans = allocVector(INTSXP, 1);
	INTEGER(ans)[0] = idmc_basin_slow_finished(R_ExternalPtrAddr(p));
	return ans;
}
